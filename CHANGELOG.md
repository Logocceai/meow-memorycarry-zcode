...
把"什么时候该用内置 `/compact` 而不是本插件"放到首屏说清楚,并附上依据报告。

### Added

- **对比报告 `docs/handoff-vs-compact.md`(中文)与 `docs/handoff-vs-compact.en.md`(英文)**。基于真实会话实测:107 次 `/compact` 调用、7 次 `/handoff` 调用、10 次 `/recall` 调用。含成本对照(非缓存输入 中位 1,347 vs 252,819 tokens;模型耗时 中位 25s vs 190s)、压缩比分层实测(压缩前 < 20k 时压缩后为 105%,> 100k 时降到 29%)、场景判定表、双方优势与劣势各一节,以及方法与局限(含 6 条未核实项)。
- **README 首屏提示(中英)**。在简介之后、正文之前明确写出:**日常长文本、只需当前窗口继续、且不需要跨窗口记忆 / 版本控制 / 本地记忆库 / 多机共享时,应当直接用内置 `/compact`**;并列出只有 `/handoff` 能做的四件事。
- **Roadmap 预告**:下一个大版本做一次"长处特化 + 短处学习 + 融合"——加深交接的强项,正面吸收 compact 的优点,补齐单位成本、时机判断、无自动触发这几条短板。方向已定,细节未定。

### Changed

- `INSTALL-FOR-AI.md`:新增「适用边界」段,并把"先讲边界再讲用法"作为安装后的**第零步**,避免用户装了才发现它解决的不是自己的问题。
- `docs/user-guide.md`:新增「什么时候不该用它:先考虑内置的 `/compact`」一节,放在「什么时候该交接」之前。
- 打包清单与核验清单纳入新报告的中英两份(25 → 27 项)。
